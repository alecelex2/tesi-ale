...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ isOpen, onOpenChange }) => {
  ...
  const playRetroSound = (type: 'click' | 'send' | 'receive') => {
    try {
      ...
      if (type === 'click') {
        // Short, crisp blip (Square wave for retro feel)
        osc.type = 'square'; 
      ...
      // Audio autoplay policy might block this on first interaction, ignore error
    }
  };
